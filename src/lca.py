...
# Adapted by Johanna de Vos (2018) to work in Python 3 and to also be applicable when using the Dutch CGN tagset

import string,os,random,glob
...
def run_lca(standard, directory, language):
    results=[]
    ...
    if language=="EN":
        ...
        noundict={}
        worddict={}
        wordlistfile=open(os.path.join("..", "lca", "bnc_all_filtered.txt"),"r")
        ...
        # reads information from celex wordlist
        adjdict={}
        verbdict={}
        noundict={}
        worddict={}
        wordlistfile=open(os.path.join("..", "lca", "celex_dutch_frequencies.txt"),"r")
        wordlist=wordlistfile.readlines()
        wordlistfile.close()
        for word in wordlist:
            wordinfo=word.strip()
            if not wordinfo in wordinfo:
                continue
            infolist=wordinfo.split()
            lemma=infolist[0]
            pos=infolist[1]
